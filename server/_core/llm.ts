...
export async function invokeLLM(env: WorkerEnv, params: InvokeParams): Promise<InvokeResult> {
  ...
  if (explicitFormat) {
    ...
    payload.response_format = { type: "json_schema", json_schema: { name: schema.name, schema: schema.schema, ...(typeof schema.strict === "boolean" ? { strict: schema.strict } : {}) } };
  }

  const apiUrl = `${env.BUILT_IN_FORGE_API_URL.replace(/\/$/, "")}/v1/chat/completions`;
